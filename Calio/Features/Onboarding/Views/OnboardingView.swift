    ...
    private var caloriesStepView: some View {
        VStack(spacing: 40) {
            ...
            VStack(spacing: 24) {
                ...
                // Calorie display with +/- buttons
                HStack(spacing: 20) {
                    // Minus button
                    Button(action: {
                        if caloriesGoal > 1200 {
                            caloriesGoal = max(1200, caloriesGoal - 50)
                            HapticManager.impact(.light)
                        }
                    }) {
                        Image(systemName: "minus.circle.fill")
                            ...
                            .scaleEffect(caloriesGoal > 1200 ? 1.0 : 0.8)
                    }
                    .disabled(caloriesGoal <= 1200)
                    
                    // Calorie display (tappable for direct input)
                    Button(action: {
                        // In a real app, this would show a number pad overlay
                        // For now, we'll just cycle through common values
                        let commonValues = [1500, 1800, 2000, 2200, 2500, 2800, 3000]
                        if let currentIndex = commonValues.firstIndex(where: { $0 >= caloriesGoal }),
                           currentIndex < commonValues.count - 1 {
                            caloriesGoal = commonValues[currentIndex + 1]
                        } else {
                            caloriesGoal = commonValues[0]
                        }
                    ...
                    // Plus button
                    Button(action: {
                        if caloriesGoal < 4000 {
                            caloriesGoal = min(4000, caloriesGoal + 50)
                            HapticManager.impact(.light)
                        }
                    }) {
                        Image(systemName: "plus.circle.fill")
                            .font(.system(size: 44))
                            .foregroundColor(.white.opacity(0.8))
                            .scaleEffect(caloriesGoal < 4000 ? 1.0 : 0.8)
                    }
                    .disabled(caloriesGoal >= 4000)
                }
                .opacity(animateElements ? 1 : 0)
                .scaleEffect(animateElements ? 1 : 0.8)
                .animation(.easeOut(duration: 0.6).delay(0.1), value: animateElements)
                ...
                VStack(spacing: 12) {
                    ...
                    HStack(spacing: 12) {
                        ForEach([1500, 2000, 2500, 3000], id: \.self) { value in
                            Button(action: {
                                caloriesGoal = value
                                HapticManager.impact(.light)
                            }) {
                                Text("\(value)")
                                    ...
                                    .foregroundColor(caloriesGoal == value ? buttonTextColor : .white)
                                    .frame(width: 70, height: 40)
                                    .background(
                                        Capsule()
                                            .fill(caloriesGoal == value ? Color.white : Color.white.opacity(0.2))
                                    )
                            }
                        }
                    }
